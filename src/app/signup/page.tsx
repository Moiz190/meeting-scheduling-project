...
import { ILoginCredential } from '../login/interface'
                    ...
                    <>
                        ...
                        >
                            ...
                            <div className="flex flex-col gap-2 mb-6">
                                <div className="mb-3">
                                    <label htmlFor="email" className="block mb-2 text-sm font-medium text-white dark:text-black">Username</label>
                                    <InputField label='Email' type='email' id='email' value={loginCreds.name} onChange={handleChangeName} />
                                </div>
                                <div>
                                    <label htmlFor="password" className="block mb-2 text-sm font-medium text-white dark:text-black">Password</label>
                                    <InputField label='Password' type='password' id='password' value={loginCreds.password} onChange={handleChangePassword} />
                                </div>
                            </div>
                            <div className='text-center mb-2'>
                                <Button label='Signup' disabled={!loginCreds.name || !loginCreds.password} />
                            </div>
                            <div className='text-xs flex justify-end items-center gap-1'>
                                <span>Already have an account?</span>
                                <Link href={'/login'}>
                                    <span
                                        className='border-b border-b-transparent hover:text-blue-600 hover:cursor-pointer pb-0.5 hover:border-b-blue-600 font-medium'>sign in</span>
                                </Link>
                            </div>
                        </form>
                    </>
